...
public class ManejadorDeEntrada {
                    ...
                    if (replay) {
                        ...
                        if (co.isTerminado()) {
                         ...
                         if (ComponenteReplay.verificarReplay(mapa, co,frame)) {
                                ...
                                cargaDePartida.empezarCarga(mapa,co, frame,asr.getRondaMayor(),false);
                            }else{
                             frame.agregarTextoAcciones("Enlace No Correcto\n");
                         }
                     }
    ...
    private ComponenteReplay co;

    public ComponenteReplay getCo() {
        return co;
    }
    
    
    public void abrirTexto(String texto, FramePrincipal frame,Mapa mapa,boolean replay) {
            ...
            try {
                ...
                AnalizadorLexicoReplays alr= new AnalizadorLexicoReplays(new StringReader(texto));
                AnalizadorSintacticoReplay asr = new AnalizadorSintacticoReplay(alr);
                asr.setFrame(frame);
                asr.parse();
                if (asr.error) {
                    ...
                    if (replay) {
                        ...
                        if (co.isTerminado()) {
                         ...
                         if (ComponenteReplay.verificarReplay(mapa, co,frame)) {
                                frame.agregarTextoAcciones("Enlace Exitoso\n");
                                CargaDePartida cargaDePartida = new CargaDePartida();
                                cargaDePartida.empezarCarga(mapa,co, frame,asr.getRondaMayor(),true);
                            }else{
                             frame.agregarTextoAcciones("Enlace No Correcto\n");
                         }
                     }
